...
We start with a rational point on the circle \\( (0,1) \\). Now imagine a line through this point \\( y = m x + 1 \\). Let's say, it intersects the x-axis at point \\( (r,0) \\). We must have \\( r = \frac{-1}{m} \\). We can rewrite the equation for the line as \\( y = 1 - \frac{x}{r} \\). If we look for intersection of this line with the circle,
